...
export class AudioRecordingService {
  sendTranscribeToServer(transcriptionText: string | null) {
    const body = { "transcripcion": transcriptionText };

    const token = localStorage.getItem('authToken');
    const auth = new HttpHeaders().set('Authorization', `Bearer ${token}`);

    return this.httpClient.post<any>(environment.apiUrlBack + 'upload-transcripcion', body, { headers: auth });
  }
